...
\begin{figure*}[tb]
 ...
 \caption{Trajectory heatmaps for all four participants under both locomotion conditions. Each row corresponds to one participant (P09--P12); the left column shows the joystick session (red--yellow heat scale) and the right column shows the teleport session (blue--cyan heat scale). Dashed white rectangles mark the five zone boundaries. Joystick sessions trace continuous, broadly distributed paths through the museum floor; teleport sessions produce sparse clusters of discrete landing points. The contrast is consistent across all four participants, reinforcing the spatial coverage finding ($p$\,=\,.009, $d$\,=\,5.27).}
